...
SYSTEM_PROMPT_VOICE = (
    ...
    "CREDENTIAL_RULE: El usuario puede guardar claves, API keys y contrasenas "
    "de forma segura con /guardar_clave (cifrado AES-256). NUNCA muestres "
    "el valor completo de una clave en tus respuestas.\n"
...
async def generate_response(text: str, chat_id: int) -> str:
    ...
    Always uses the tools path (no fast path) to ensure access to
    search_second_brain and avoid hallucinations.
    """
    ...
    logger.info(
        ...
        chat_id, get_correlation_id(), len(TOOLS_DEFINITIONS),
        len(trimmed_history), len(text),
    )
    ...
    try:
        ...
        return "Lo siento, el modelo tardo demasiado en responder. Intenta con un mensaje mas corto."
    ...
    logger.info(
        ...
        _elapsed, len(content or ""), bool(tool_calls),
    )
    ...
    if tool_calls:
        ...
        for tc in tool_calls:
            ...
            logger.info(
                ...
                chat_id, func_name, str(args)[:200],
            )
    ...
    if content:
        await db.save_chat_message(
            chat_id, MessageRole.assistant.value, content[:2000]
        )
