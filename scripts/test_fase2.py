...
sys.modules["src.logger"].logger = type(
    "Logger", (), {"info": print, "warning": print, "error": print, "debug": print, "exception": print}
)()

from src.utils.vault_indexer import parse_frontmatter, chunk_by_headings, estimate_tokens, build_obsidian_uri
...
print("=" * 60)
print(f"Frontmatter: type={metadata2.get('type')}, tags={metadata2.get('tags')}, status={metadata2.get('status')}")
